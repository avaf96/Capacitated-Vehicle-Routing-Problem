...
def SA_algorithm():
    ...
    best_solution_process.append(current_solution_cost)
    while (current_temp >= final_temp and exec_time < 20):
        cust_range = cust_range0
        while(cust_range>=0):
            neighbour_solution , neighbour_solution_cost = neighbourhood(current_solution)
            solution_count +=1
            deltaE = neighbour_solution_cost - current_solution_cost
            if deltaE <= 0:
                current_solution.clear()
                current_solution = copy.deepcopy(neighbour_solution)
                current_solution_cost = neighbour_solution_cost
                best_solution_process.append(current_solution_cost)
            elif random.uniform(0,1) < math.exp(-deltaE/current_temp):
                current_solution.clear()
                current_solution = copy.deepcopy(neighbour_solution)
                current_solution_cost = neighbour_solution_cost
                best_solution_process.append(current_solution_cost)
        ...
        cust_range0 = cust_range0 * beta
        end_time = time.time()
        exec_time = int(round(end_time - start_time))/60
        
    # plot_solution_selection_process(best_solution_process)
    # visualRoutes(current_solution)
    return current_solution,current_solution_cost,solution_count,initial_cost




# # header = ("instance" , "bestSol_initial_cost", "best" , "worst" , "avg_result" , "avg_cpu_time", "best_sol_eval" ,"gap" )
# # with open('C:\\Users\\Desktop\\algo\\SA_result.csv', 'w', encoding='UTF8', newline='') as f:
# #     writer = csv.writer(f)
...
#         start_time = time.time()
#         current_solution_cost,solution_count,initial_cost = SA_algorithm()
#         result.append(current_solution_cost)
...
# # with open('C:\\Users\\Desktop\\algo\\SA_result.csv', 'a', encoding='UTF8', newline='') as f:
# #     writer = csv.writer(f)
...
header = ("instance" , "initial_cost","best_solution_cost" , "best_sol_eval" ,"gap")
with open('C:\\Users\\Desktop\\algo\\SA1_result.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(header)
dataa = []
d = "C:\\Users\\Desktop\\algo\\STD-Ins"
for path in os.listdir(d):
    ...
    start_time = time.time()
    best_solution,best_solution_cost,best_sol_eval,initial_cost = SA_algorithm()
    end_time = time.time()
    ...
    with open('C:\\Users\\Desktop\\algo\\SA1_result.csv', 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(dataa1)

with open('C:\\Users\\Desktop\\algo\\SA2_result.csv', 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(dataa)
